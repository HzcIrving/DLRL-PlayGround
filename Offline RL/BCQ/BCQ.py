...
class BCQ(object):
    ...
    def select_action(self,state):
        with torch.no_grad():
            # e.g.: state (32,5)
            #       reshape(1,-1) -> state:(1,160)
            #       repeat(100,1) -> state:(100,160) (100:采样的次数,sample n Actions)
            state = torch.FloatTensor(state.reshape(1, -1)).repeat(100, 1).to(self.device)

            """ a_tilde = D(s,z;\omega_2) """
            a_tilde = self.vae.decode(state)
            action = self.actor(state,a_tilde)
    ...
    def train(self, replay_buffer, iterations, batch_size=100):
        for it in range(iterations):
            ...
            # Constrain the KL Divergence between 标准高斯 and (mu,std)
            ...
            # Critic Training
            with torch.no_grad():
                # Duplicate next state 10 times in dim 0
                # 100 -> 1000
