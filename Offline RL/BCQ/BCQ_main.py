...
# WRITER = SummaryWriter(logdir=Log_path+datetime.datetime.now().strftime('%Y%m%d_%H%M%S'))

# Handles interactions with the environment, i.e. train behavioral or generate buffer
def DDPG_Training(env, state_dim, action_dim, max_action, device, args):
	# For saving files
	setting = f"{args.env}_{args.seed}"
	buffer_name = f"{args.buffer_name}_{setting}"

	# Initialize and load policy
	policy = OffPolicy_DDPG.DDPG(state_dim, action_dim, max_action, device)#, args.discount, args.tau)
	...
	for t in range(int(args.max_timesteps)):
		...
		# DDPG _ Select action with noise
		if (
		...
		# WRITER.add_scalar('Accum_Reward/DDPG_reward',episode_reward,episode_timesteps)

		# Train agent after collecting sufficient data
		if args.train_behavioral and t >= args.start_timesteps:
			policy.train(replay_buffer, args.batch_size) # DDPG Training ...
...
if __name__ == "__main__":
	...
	if args.train_behavioral or args.generate_buffer:
		DDPG_Training(env, state_dim, action_dim, max_action, device, args)
	else:
