...
"""
Spinning Up Original Code
"mlp": General Components
"AWActorCritic": AWAC Core
"AWACActor": AWAC Actor
"SACActor": SAC Actor
"MLPQFunction": Critic
"MLPVFunction": Value Neet
"""
...
class AWActorCritic(nn.Module):
    def __init__(self,observation_space,action_space,hidden_sizes=(256,256),awac_hidden_sizes=(256,256,256,256), activation=nn.ReLU, special_policy="awac"):
        super(AWActorCritic, self).__init__()
        ...
        self.pi = AWACActor(obs_dim,act_dim,awac_hidden_sizes,activation,act_limit).to(device)
        # else:
        #     SAC: Squashed Gaussian MLP Actor
            # self.pi = SACActor(obs_dim, act_dim, hidden_sizes, activation, act_limit).to(device)
        ...
        # V -- Target V (SAC1)
        self.V = MLPVFunction(obs_dim, hidden_sizes, activation).to(device)
...
class AWACActor(nn.Module):
    ...
    def forward(self,obs,deterministic=False,with_logprob=True):
        ...
        pi_distribution = Normal(mu, std) # log π(a|s)
        if deterministic:
            pi_action = mu # eval stage
        else:
...
"""SAC SquashedGaussian MLP Actor"""
LOG_STD_MAX = 2
LOG_STD_MIN = -20
